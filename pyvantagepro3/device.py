...
class VantagePro2:
    """Communicates with the station by sending commands, reads the binarydata and parsing it into usable scalar values.
    ...
    def _get_archives_generator(self, start_date=None, stop_date=None):
        ...
        minutes = start_date.minute % period  # pyright: ignore[reportOperatorIssue]
    ...
    def _check_revision(self):
        ...
        if self.firmware_date < date:  # pyright: ignore[reportOperatorIssue]
            self.RevB = False
        else:
            self.RevA = False
